...
function IncomeProvider:onBind(target)
    assert(self.m_Owner == nil, "IncomeProvider:onBind() the component has already bound a target.")
...
function IncomeProvider:onUnbind()
    assert(self.m_Owner ~= nil, "IncomeProvider:onUnbind() the component has not bound to a target.")
